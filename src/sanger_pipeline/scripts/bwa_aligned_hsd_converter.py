...
class BWAAlignedHSDConverter:
            ...
            try:
                result = subprocess.run(
                    [
                        ...
                        "-k",
                        "15",  # Minimum seed length
                        "-r",
                        "1.5",  # Re-seeding trigger
                        self.reference_file,
                        temp_fasta,
                    ],
                    capture_output=True,
    ...
    def parse_sam_for_variants(self, sam_output: str, hvs_region: str) -> List[str]:
        ...
        for line in sam_output.split("\n"):
            ...
            variants.extend(
                self._extract_variants_from_cigar(
    ...
    def _extract_variants_from_cigar(
        self, pos: int, cigar: str, seq: str, region_start: int, region_end: int
    ) -> List[str]:
        """
                ...
                for i in range(length):
                    ...
                    if region_start <= current_ref_pos <= region_end:
                        if current_seq_pos < len(seq):
                            ...
                            if (
                                seq_base in "ATCG"
                                and ref_base in "ATCG"
                                and seq_base != ref_base
                            ):
            ...
            try:
                ...
                with open(fasta_file, "r") as f:
                    for record in SeqIO.parse(f, "fasta"):
                        ...
                        if len(sequence) < 50:
                            print(
                                f"Skipping {fasta_file.name}: Sequence too short after primer removal ({len(sequence)}bp)"
                            )
                            continue

                        # Align with BWA (primer-free sequence)
                        sam_output = self.align_sequence_with_bwa(
                            sequence, f"{sample_id}_{hvs_region}"
                        )
                        ...
                        if variants:
                            ...
                            print(
                                f"📁 {fasta_file.name}: {len(variants)} variants ({hvs_region}) - BWA aligned (primers removed)"
                            )
                        else:
                            print(
                                f"📁 {fasta_file.name}: No variants found ({hvs_region}) - BWA aligned (primers removed)"
                            )
...
def main():
    ...
    if len(sys.argv) != 3:
        ...
        print(
            "Example: python bwa_aligned_hsd_converter.py output_min30_q30/consensus/ output.hsd"
        )
        sys.exit(1)
