...
class HVSConsensusToHSDConverter:
            ...
            for i in range(min(len(reverse_primer) + 5, len(seq_upper))):
                ...
                if start_pos >= 0:
                    ...
                    if len(window) == len(reverse_primer):
                        matches = sum(
                            1 for a, b in zip(window, reverse_primer) if a == b
                        )
                        if (
                            matches >= len(reverse_primer) - 2
    ...
    def find_variants(self, sample_seq: str, hvs_region: str) -> List[str]:
        ...
        aligned_sample = sample_seq[
            best_alignment_pos : best_alignment_pos + len(ref_clean)
        ]
        ...
        for pos in range(min_length):
            ...
            if sample_base in "ATCG" and ref_base in "ATCG" and ref_base != sample_base:
    ...
    def _find_alignment_position(self, sample_seq: str, ref_seq: str) -> int:
        ...
        window_size = min(
            50, len(ref_seq) // 3
        )  # Use first third of reference for alignment
        ...
        for start_pos in range(len(sample_seq) - window_size + 1):
            ...
            for i in range(window_size):
                if (
                    i < len(ref_window)
                    and i < len(sample_window)
                    and sample_window[i] in "ATCG"
                    and ref_window[i] in "ATCG"
                ):
                    total_clear += 1
                ...
                with open(fasta_file, "r") as f:
                    for record in SeqIO.parse(f, "fasta"):
                        ...
                        if len(sample_seq) < 50:
                            print(
                                f"Skipping {fasta_file.name}: Sequence too short after primer removal ({len(sample_seq)}bp)"
                            )
                            continue
                        ...
                        if variants:
                            ...
                            print(
                                f"📁 {fasta_file.name}: {len(variants)} variants ({hvs_region})"
                            )
                        else:
                            print(
                                f"📁 {fasta_file.name}: No variants found ({hvs_region})"
                            )
    ...
    def get_actual_range(
        self, sample_variants: List[str], sample_regions: List[str]
    ) -> str:
        """
        ...
        return (
            ";".join(ranges) if ranges else "16024-16365"
        )  # Default to HVS1 if nothing detected
    ...
    def write_hsd_file(self, sample_variants: Dict[str, List[str]], output_file: str):
        ...
        with open(output_file, "w") as f:
            for sample_id, variants in sample_variants.items():
                ...
                actual_range = self.get_actual_range(
                    variants, sample_regions[sample_id]
                )
...
def main():
    if len(sys.argv) != 3:
        print(
            "Usage: python convert_hvs_consensus_to_hsd.py <consensus_directory> <output_hsd_file>"
        )
        sys.exit(1)
