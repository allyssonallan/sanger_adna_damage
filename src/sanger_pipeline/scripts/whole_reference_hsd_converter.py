...
class WholeReferenceHSDConverter:
    ...
    def align_to_whole_reference(self, consensus_seq: str, sample_name: str) -> Optional[Dict]:
        """
        ...
        try:
            ...
            alignment_info = self._parse_whole_reference_alignment(temp_output_path, sample_name)
    ...
    def _parse_whole_reference_alignment(self, alignment_file: str, sample_name: str) -> Dict:
        """
        ...
        try:
            ...
            ref_start_pos = self._find_alignment_start_position(ref_aligned, cons_aligned)
            
            # Determine which HVS region(s) this sequence overlaps
            matching_regions = self._determine_hvs_regions(ref_start_pos, len(cons_aligned.replace('-', '')))
    ...
    def _find_alignment_start_position(self, ref_aligned: str, cons_aligned: str) -> int:
        """
    ...
    def extract_variants_from_whole_alignment(self, alignment_info: Dict, sample_name: str) -> List[str]:
        """
        ...
        for i in range(len(ref_aligned)):
            ...
            if len(variants) >= max_variants:
                print(f"⚠️  {sample_name}: Too many variants found ({len(variants)}), stopping")
                break
            ...
            if ref_base == '-':
                ...
                if cons_base in 'ATCG' and current_ref_pos <= 16569:  # Valid mtDNA position
                    variant = f"{current_ref_pos}{cons_base}"
                    variants.append(variant)
                current_ref_pos += 1
        
        print(f"📍 {sample_name}: Found {len(variants)} variants from whole reference alignment")
        return variants
    
    def process_consensus_file(self, fasta_file: Path) -> Tuple[str, List[str], List[str]]:
        """
            ...
            with open(fasta_file, 'r') as f:
                for record in SeqIO.parse(f, "fasta"):
                    ...
                    if len(consensus_seq) < 50:
                        print(f"⚠️  {fasta_file.name}: Sequence too short ({len(consensus_seq)}bp)")
                        return sample_id, [], []
                    
                    # Align to whole reference
                    alignment_info = self.align_to_whole_reference(consensus_seq, sample_id)
                    ...
                    # Extract variants
                    variants = self.extract_variants_from_whole_alignment(alignment_info, sample_id)
                    
                    print(f"🎯 {fasta_file.name}: Detected regions {detected_regions}, {len(variants)} variants")
                    
                    if region_hint and region_hint not in detected_regions:
                        print(f"⚠️  {fasta_file.name}: Filename suggests {region_hint}, but aligned to {detected_regions}")
    ...
    def convert_directory(self, consensus_dir: str, output_file: str) -> None:
        ...
        for fasta_file in sorted(fasta_files):
            sample_id, detected_regions, variants = self.process_consensus_file(fasta_file)
        ...
        with open(output_file, 'w') as f:
            ...
            for sample_id, data in sorted(sample_data.items()):
                if data['variants']:
                    ...
                    unique_variants.sort(key=lambda x: int(re.findall(r'\\d+', x)[0]) if re.findall(r'\\d+', x) else 0)
                    ...
                    print(f"✅ {sample_id}: {len(unique_variants)} variants ({regions_str})")
