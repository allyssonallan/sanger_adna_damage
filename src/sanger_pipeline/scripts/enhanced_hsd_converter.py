...
class EnhancedHSDConverter:
    ...
    def __init__(self, reference_file: str = "ref/rCRS.fasta", alignment_tool: str = "mafft"):
        """
    ...
    def load_reference(self):
        ...
        try:
            ...
            if not reference_path.exists():
                ...
                reference_path = Path(__file__).parent.parent.parent.parent / self.reference_file
    ...
    def create_alignment(self, sequences: List[str], temp_dir: str) -> Optional[str]:
        ...
        try:
            ...
            print(f"Alignment tool '{self.alignment_tool}' not found. Please install it.")
            return None
    ...
    def compare_sequences_aligned(self, sample_seq: str, region: str) -> List[str]:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            alignment_file = self.create_alignment([reference_region, sample_seq], temp_dir)
            ...
            try:
                ...
                return self.extract_mutations_from_alignment(ref_aligned, sample_aligned, region)
                
            except Exception as e:
                print(f"Error reading alignment: {e}")
                return self.compare_sequences_direct(sample_seq, region)
    
    def extract_mutations_from_alignment(self, ref_aligned: str, sample_aligned: str, region: str) -> List[str]:
        ...
        while i < len(ref_aligned) and i < len(sample_aligned):
            ...
            if ref_base == '-':
                ...
                if ref_base.upper() != sample_base.upper():
                    mutations.append(f"{ref_pos}{ref_base.upper()}>{sample_base.upper()}")
                i += 1
        
        return mutations
    
    def convert_to_hsd(self, consensus_dir: str, output_file: str, method: str = "direct"):
        """
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: python enhanced_hsd_converter.py <consensus_directory> <output_hsd_file> [--method=aligned]")
        print("Methods:")
