            ...
            for hvs_region, primers in converter.primers.items():
                ...
                if forward_primer in sequence.upper():
                    ...
                    for i in range(min(len(forward_primer) + 10, len(sequence))):
                        ...
                        if len(window) == len(forward_primer):
                            matches = sum(
                                1 for a, b in zip(window, forward_primer) if a == b
                            )
                    ...
                    print(
                        f"      🔍 Best forward match: {best_match}/{len(forward_primer)} ({match_percent:.1f}%) at position {best_pos}"
                    )
                    if best_pos >= 0:
                        window = sequence[
                            best_pos : best_pos + len(forward_primer)
                        ].upper()
                ...
                if reverse_primer in sequence.upper():
                    ...
                    for i in range(min(len(reverse_primer) + 10, len(sequence))):
                        ...
                        if start_pos >= 0:
                            window = sequence[
                                start_pos : start_pos + len(reverse_primer)
                            ].upper()
                    ...
                    print(
                        f"      🔍 Best reverse match: {best_match}/{len(reverse_primer)} ({match_percent:.1f}%) at position {best_pos}"
                    )
                    if best_pos >= 0:
                        window = sequence[
                            best_pos : best_pos + len(reverse_primer)
                        ].upper()
                        print(f"      Sequence: {window}")
