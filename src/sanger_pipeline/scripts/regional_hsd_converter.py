...
class RegionalHSDConverter:
    ...
    def align_to_reference_segment(self, consensus_seq: str, hvs_region: str) -> Optional[str]:
        """
    ...
    def find_variants_from_alignment(self, aligned_consensus: str, hvs_region: str) -> List[str]:
        """
        ...
        max_differences = min_length // 8  # Allow max 12.5% differences (more conservative)
        differences_found = 0
        ...
        try:
            ...
            with open(fasta_file, 'r') as f:
                for record in SeqIO.parse(f, "fasta"):
                    ...
                    aligned_consensus = self.align_to_reference_segment(consensus_seq, hvs_region)
                    ...
                    variants = self.find_variants_from_alignment(aligned_consensus, hvs_region)
                    
                    print(f"📁 {fasta_file.name}: {len(variants)} variants ({hvs_region}) - aligned method")
