...
class FastaToHSDConverter:
    ...
    def _extract_hvs_references(self, hvs_regions: List[str]) -> str:
        ...
        for region in hvs_regions:
            if region in self.hvs_regions:
                ...
                print(
                    f"  Added {region}: positions {start+1}-{end} ({len(ref_segment)} bp)"
                )
    ...
    def find_variants(self, sample_seq: str, sample_name: str) -> List[str]:
        ...
        if len(hvs_regions) == 1:
            ...
            variants = self._find_variants_single_region(
                sample_clean, region, ref_seq, sample_name
            )
        else:
            # Multiple HVS regions - try to detect boundaries in the concatenated sequence
            variants = self._find_variants_concatenated(
                sample_clean, hvs_regions, sample_name
            )

        print(f"  Found {len(variants)} variants for {sample_name}")
        return variants

    def _find_variants_single_region(
        self, sample_seq: str, region: str, ref_seq: str, sample_name: str
    ) -> List[str]:
        ...
        for pos in range(min_length):
            ...
            if ref_base != sample_base:
                ...
                if variant_count <= 5:
                    print(
                        f"    Variant {variant_count}: pos {mt_position} {ref_base}→{sample_base}"
                    )

        return variants

    def _find_variants_concatenated(
        self, sample_seq: str, hvs_regions: List[str], sample_name: str
    ) -> List[str]:
        ...
        for region in hvs_regions:
            if region in self.hvs_regions:
                region_ref = self.reference_seq[
                    self.hvs_regions[region]["start"]
                    - 1 : self.hvs_regions[region]["end"]
                ]
        ...
        if len(sample_seq) < total_ref_length * 0.5:  # Sample is too short
            print(
                f"    Warning: Sample sequence ({len(sample_seq)}bp) much shorter than expected ({total_ref_length}bp)"
            )
            return variants
        ...
        for i, (region, ref_seq, mt_start) in enumerate(
            zip(hvs_regions, ref_sequences, region_starts)
        ):
            ...
            for pos in range(comparison_length):
                ...
                if ref_base != sample_base:
                    ...
                    if region_variants <= 3:
                        print(
                            f"    {region} Variant {region_variants}: pos {mt_position} {ref_base}→{sample_base}"
                        )
    ...
    def convert_pipeline_output(
        self, pipeline_output_dir: Path, output_file: Path
    ) -> None:
        """
            ...
            try:
                for record in SeqIO.parse(fasta_file, "fasta"):
                    ...
                    if sample_name == "consensus" or not sample_name:
                        ...
                        sample_name = (
                            fasta_file.stem.replace("_merged", "")
                            .replace("_final", "")
                            .replace("_consensus", "")
                        )
...
def main():
    ...
    parser.add_argument("--output", "-o", required=True, help="Output HSD file")
    parser.add_argument(
        "--reference",
        "-r",
