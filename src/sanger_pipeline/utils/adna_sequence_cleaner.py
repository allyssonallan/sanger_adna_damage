...
class aDNASequenceCleaner:
    ...
    def clean_fasta_file(self, input_file: str, output_file: str, 
                        aggressive_cleaning: bool = False) -> None:
        """
        ...
        for record in SeqIO.parse(input_file, "fasta"):
            ...
            # Apply filters
            if (len(cleaned_seq) >= self.min_length and 
                cleaned_quality >= self.min_quality):
                ...
                logger.info(f"Kept {record.id}: {len(original_seq)}bp -> {len(cleaned_seq)}bp, "
                          f"Quality: {original_quality:.3f} -> {cleaned_quality:.3f}")
            else:
                logger.warning(f"Filtered {record.id}: Length={len(cleaned_seq)}, "
        ...
        logger.info(f"Cleaning completed: {kept_sequences}/{total_sequences} sequences kept")
...
def main():
    ...
    parser.add_argument("-q", "--min-quality", type=float, default=0.6,
