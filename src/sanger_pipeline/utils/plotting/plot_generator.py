...
class PlotGenerator:
    ...
    def create_damage_correlation_plot(self, damage_data: List[Dict]) -> Optional[str]:
        ...
        try:
            ...
            if len(overall_damage) > 1:
                ...
                p = np.poly1d(z)
                ax1.plot(
                    sorted(overall_damage),
                    p(sorted(overall_damage)),
                    "r--",
                    alpha=0.8,
                    label=f"Trend (r={np.corrcoef(overall_damage, valid_percentage)[0,1]:.2f})",
                )
                ax1.legend()
            ...
            ax4.hist(
                [low_damage_quality, high_damage_quality],
                bins=15,
                alpha=0.7,
                label=["Low Damage (≤15%)", "High Damage (>15%)"],
    ...
    def create_status_summary_plot(self, damage_data: List[Dict]) -> Optional[str]:
        ...
        try:
            ...
            if quality_tiers:
                colors_q = [
                    self.colors["high_quality"],
                    self.colors["significant"],
                    self.colors["damage"],
                    self.colors["background"],
                ]
                ax1.pie(
                    quality_tiers.values(),
                    labels=quality_tiers.keys(),
                    autopct="%1.1f%%",
                    colors=colors_q[: len(quality_tiers)],
                )
            ...
            if damage_tiers:
                ...
                ax2.pie(
                    damage_tiers.values(),
                    labels=damage_tiers.keys(),
                    autopct="%1.1f%%",
                    colors=colors_d[: len(damage_tiers)],
                )
            ...
            for d in damage_data:
                level = self._get_confidence_level(
                    d.get("p_value_5_prime", 1.0), d.get("p_value_3_prime", 1.0)
                )
            ...
            summary_data = {
                ...
                "High Quality": len(
                    [d for d in damage_data if d["valid_percentage"] > 75]
                ),
                "Significant Damage": len(
                    [d for d in damage_data if d["overall_damage_rate"] > 10]
                ),
                "High Confidence": len(
                    [
                        d
    ...
    def create_quality_damage_plot(self, damage_data: List[Dict]) -> Optional[str]:
        ...
        try:
            ...
            # Add quality zones
            ax1.axhline(
                y=90, color="green", linestyle="--", alpha=0.5, label="Excellent (>90%)"
            )
            ax1.axhline(
                y=75, color="blue", linestyle="--", alpha=0.5, label="Good (>75%)"
            )
            ax1.axhline(
                y=50, color="orange", linestyle="--", alpha=0.5, label="Fair (>50%)"
            )
            ax1.legend()
