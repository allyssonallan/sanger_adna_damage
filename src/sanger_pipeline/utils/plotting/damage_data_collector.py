    ...
    def _parse_damage_json(self, data: Dict, sample_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'confidence_5_prime': statistical_tests.get('confidence_5_prime', 'low'),
                'confidence_3_prime': statistical_tests.get('confidence_3_prime', 'low'),
    ...
    def get_dashboard_data(self, base_output_dir: str) -> Dict[str, Any]:
        ...
        if samples_data:
            summary = {
                ...
                'high_damage_samples': len([s for s in samples_data if s['damage_tier'] in ['high', 'extreme']]),
                'high_quality_samples': len([s for s in samples_data if s['quality_tier'] in ['excellent', 'good']]),
                'significant_samples': len([s for s in samples_data if s['confidence_level'] in ['high', 'very_high']])
            }
