...
class DamagePlotCoordinator:
    ...
    def generate_all_plots(self, output_dir: Optional[str] = None) -> Dict[str, str]:
        ...
        try:
            plots['damage_distribution'] = self.plot_generator.create_damage_distribution_plot(
                damage_data
            )
    ...
    def _generate_summary_stats(self, dashboard_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        avg_valid = sum(valid_percentages) / len(valid_percentages) if valid_percentages else 0
...
def main():
    ...
    parser.add_argument('--plot-type', choices=['all', 'damage', 'correlation', 'status', 'quality'],
