...
class ImprovedFastaToHSDConverter:
    ...
    def __init__(self, reference_path: str = "ref/rCRS.fasta", min_quality_threshold: float = 0.7):
        """
    ...
    def _load_reference(self):
        ...
        try:
            with open(self.reference_path, 'r') as f:
                ...
                logger.info(f"Reference sequence loaded: {len(self.reference_seq)} bases")
    ...
    def _calculate_sequence_quality(self, sequence: str) -> float:
        ...
        valid_count = sum(1 for base in sequence.upper() if base in self.valid_nucleotides)
    ...
    def _detect_hvs_regions(self, sequence: str, sample_name: str) -> List[str]:
        ...
        logger.info(f"Processing {sample_name}: Sample={seq_len}bp, Quality={overall_quality:.2f}")
        
        if overall_quality < self.min_quality_threshold:
            logger.warning(f"Low quality sequence for {sample_name}: {overall_quality:.2f}")
            return detected_regions
        
        # Check for HVS regions based on sequence length and content
        if seq_len >= 300:  # Likely HVS1 or combined regions
            if any(name.upper() in sample_name.upper() for name in ['HVS1', 'CONSENSUS']):
    ...
    def _compare_sequences_with_quality_control(self, sequence: str, regions: List[str]) -> List[str]:
        """
    ...
    def _find_variants_in_region(self, sequence: str, ref_region: str, start_pos: int, region_name: str) -> List[str]:
        """
    ...
    def convert_fasta_to_hsd(self, input_fasta: str, output_hsd: str, 
                           quality_filter: bool = True) -> None:
        """
        ...
        with open(output_hsd, 'w') as hsd_file:
            ...
            for record in SeqIO.parse(input_fasta, "fasta"):
                ...
                if quality_filter and quality_score < self.min_quality_threshold:
                    logger.warning(f"Skipping {sample_name}: quality too low ({quality_score:.2f})")
                    skipped_count += 1
                    continue
                ...
                variants = self._compare_sequences_with_quality_control(sequence, detected_regions)
                ...
                if variants:
                    ...
                    logger.info(f"Processed {sample_name}: {len(variants)} variants, Quality: {quality_score:.2f}")
                    processed_count += 1
                else:
                    logger.warning(f"No variants found for {sample_name}")
                    skipped_count += 1
        
        logger.info(f"Conversion completed: {processed_count} sequences processed, {skipped_count} skipped")
...
def main():
    ...
    parser.add_argument("-r", "--reference", default="ref/rCRS.fasta", help="Reference sequence file")
    parser.add_argument("-q", "--quality-threshold", type=float, default=0.7, 
