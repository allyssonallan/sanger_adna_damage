...
class HSDDiversityAnalyzer:
    ...
    def analyze_diversity(self, samples: list) -> dict:
        ...
        analysis['variant_statistics'] = {
            ...
            'mean_variants': sum(variant_counts) / len(variant_counts) if variant_counts else 0,
    ...
    def _analyze_position_diversity(self, samples: list) -> dict:
        ...
        for position, mutations in position_mutations.items():
            ...
            diversity_stats[position] = {
                ...
                'diversity_score': len(mutation_counts) / len(mutations) if mutations else 0
            }
    ...
    def _analyze_sample_similarity(self, samples: list) -> dict:
        ...
        for i in range(len(samples)):
            for j in range(i + 1, len(samples)):
                ...
                variants1 = set(f"{pos}{mut}" for pos, mut in sample1['variants'].items())
                variants2 = set(f"{pos}{mut}" for pos, mut in sample2['variants'].items())
        ...
        return {
            ...
            'mean_similarity': sum(jaccard_scores) / len(jaccard_scores) if jaccard_scores else 0,
    ...
    def _detect_potential_issues(self, samples: list) -> list:
        ...
        if 'pairwise_similarities' in similarities:
            high_similarity = [s for s in similarities['pairwise_similarities'] 
    ...
    def generate_report(self, hsd_file: str, output_file: str | None = None) -> str:
        ...
        report_lines.append(f"    Min: {analysis['variant_statistics']['min_variants']}")
        report_lines.append(f"    Max: {analysis['variant_statistics']['max_variants']}")
        report_lines.append(f"    Mean: {analysis['variant_statistics']['mean_variants']:.1f}")
        report_lines.append(f"  Total unique positions: {analysis['variant_statistics']['total_unique_positions']}")
        report_lines.append("")
        ...
        if 'mean_similarity' in similarities:
            report_lines.append(f"  Mean sample similarity: {similarities['mean_similarity']:.3f}")
            report_lines.append(f"  Min sample similarity: {similarities['min_similarity']:.3f}")
            report_lines.append(f"  Max sample similarity: {similarities['max_similarity']:.3f}")
        report_lines.append("")
        
        # Most variable positions
        position_div = analysis['position_diversity']
        if position_div:
            sorted_positions = sorted(position_div.items(), 
            ...
            for position, data in sorted_positions[:10]:
                mutations = ', '.join(f"{mut}({count})" for mut, count in data['mutations'].items())
                report_lines.append(f"  Position {position}: {mutations} (diversity: {data['diversity_score']:.3f})")
            report_lines.append("")
        ...
        if similarities.get('mean_similarity', 0) > 0.5:
            report_lines.append("  - High similarity detected - check for contamination or alignment artifacts")
            report_lines.append("  - Consider using stricter quality filters")
            report_lines.append("  - Verify reference sequence alignment")
        else:
            report_lines.append("  - Good diversity detected - samples appear genetically distinct")
            report_lines.append("  - Consider uploading to HaploGrep for haplogroup classification")
