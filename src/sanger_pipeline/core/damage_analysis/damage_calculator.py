...
class DamageCalculator:
    ...
    def calculate_damage_statistics(
        ...
        ref_clean, query_clean = self.aligner.clean_alignment(
        ...
        valid_bases = sum(
            1
            for r, q in zip(ref_clean, query_clean)
            if r.upper() in "ACGT" and q.upper() in "ACGT"
        )
        n_content = query_clean.upper().count("N")
        ambiguous_content = sum(
            1 for base in query_clean.upper() if base not in "ACGTN"
        )
        ...
        if valid_bases > 0:
            overall_damage_rate = (
                total_ct_transitions + total_ga_transitions
            ) / valid_bases
        ...
        return {
            ...
            "sequence_quality": {
                "n_percentage": (
                    (n_content / total_bases * 100) if total_bases > 0 else 0
                ),
                "valid_percentage": (
                    (valid_bases / total_bases * 100) if total_bases > 0 else 0
                ),
            },
    ...
    def calculate_positional_damage(
        ...
        for i in range(min(self.terminal_length, len(ref_clean))):
            ...
            if i < len(ref_clean):
                is_damage_5 = (
                    ref_clean[i].upper() == "C" and query_clean[i].upper() == "T"
                ) or (ref_clean[i].upper() == "G" and query_clean[i].upper() == "A")
            ...
            if pos_3 >= 0:
                is_damage_3 = (
                    ref_clean[pos_3].upper() == "C"
                    and query_clean[pos_3].upper() == "T"
                ) or (
                    ref_clean[pos_3].upper() == "G"
                    and query_clean[pos_3].upper() == "A"
                )
                damage_3_prime.append(1.0 if is_damage_3 else 0.0)

        return {"5_prime": damage_5_prime, "3_prime": damage_3_prime}
    ...
    def _calculate_terminal_damage(
        ...
        valid_positions = sum(
            1
            for r, q in zip(ref_region, query_region)
            if r.upper() in "ACGT" and q.upper() in "ACGT"
        )

        if valid_positions == 0:
            logger.warning(
                f"No valid bases in {terminal} terminal region for damage analysis"
            )
            return 0.0
        ...
        logger.debug(
            f"{terminal} terminal: {valid_positions}/{len(ref_region)} valid positions, "
    ...
    def _count_transitions(
        ...
        for ref_base, query_base in zip(ref_seq, query_seq):
            ...
            if ref_base.upper() not in "ACGT" or query_base.upper() not in "ACGT":
                continue
