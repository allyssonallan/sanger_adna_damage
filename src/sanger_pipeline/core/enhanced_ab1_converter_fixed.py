...
class EnhancedAB1Converter:
    ...
    def __init__(
        ...
        self.primers = self._setup_primer_pairs(custom_primers_forward, custom_primers_reverse)
    ...
    def _setup_primer_pairs(self, custom_forward: Optional[Dict[str, str]] = None, 
                           custom_reverse: Optional[Dict[str, str]] = None) -> Dict[str, Dict[str, str]]:
        """
        ...
        for region, primer_pair in self.default_primers.items():
            if 'forward' not in primer_pair or 'reverse' not in primer_pair:
                raise ValueError(f"Missing forward or reverse primer for region {region}: {primer_pair}")
            primers[region] = {
                'forward': primer_pair['forward'],
                'reverse_complement': self._reverse_complement(primer_pair['reverse']),
                'reverse_original': primer_pair['reverse']  # Keep original for reference
            }
        ...
        if custom_reverse:
            for region, reverse_seq in custom_reverse.items():
                if region not in primers:
                    primers[region] = {}
                primers[region]['reverse_complement'] = self._reverse_complement(reverse_seq.upper())
    ...
    def detect_primer_orientation(self, sequence: str, region: str) -> Dict[str, Any]:
        ...
        forward_score = self._find_best_primer_match(seq_upper, forward_primer, from_start=True)
        forward_pos = self._find_primer_position(seq_upper, forward_primer, from_start=True)
        
        # Check reverse complement at end
        reverse_score = self._find_best_primer_match(seq_upper, reverse_complement, from_start=False)
        reverse_pos = self._find_primer_position(seq_upper, reverse_complement, from_start=False)
    ...
    def _find_primer_position(self, sequence: str, primer: str, from_start: bool = True) -> int:
        """
    ...
    def remove_primers_with_orientation(self, sequence: str, region: str) -> Tuple[str, Dict]:
        """
        ...
        if not self.enable_primer_removal or region not in self.primers:
            return sequence, {"primers_removed": False, "reason": "disabled_or_unknown_region"}
        ...
        if primer_info["forward_score"] >= 0.4 and primer_info["forward_position"] >= 0:
            forward_end = primer_info["forward_position"] + len(primer_info["forward_primer"])
        ...
        removal_info["primers_removed"] = removal_info["forward_removed"] or removal_info["reverse_removed"]
        removal_info["final_length"] = len(cleaned_seq)
        removal_info["length_reduction"] = removal_info["original_length"] - removal_info["final_length"]
        
        return cleaned_seq, removal_info

    def analyze_extremity_n_abundance(self, sequence: str, stage: str = "unknown") -> Dict:
        """
        ...
        if seq_len < analysis_len * 2:
            return {
                ...
                "total_n_fraction": sequence.count('N') / seq_len if seq_len > 0 else 0.0,
    ...
    def _classify_damage_pattern(self, five_prime_n: float, three_prime_n: float, total_n: float) -> str:
        """
    ...
    def adjust_parameters_for_damage(self, n_analysis: Dict) -> Dict:
        ...
        if damage_pattern == "severe_degradation":
            ...
            adjusted_params["quality_window_size"] = max(5, self.quality_window_size - 3)
            adjusted_params["quality_threshold_fraction"] = max(0.5, self.quality_threshold_fraction - 0.2)
            adjusted_params["primer_similarity_threshold"] = 0.25
            
        elif damage_pattern in ["five_prime_bias", "three_prime_bias", "moderate_degradation"]:
            # Moderately lenient for damaged DNA
            adjusted_params["min_quality"] = max(17, self.min_quality - 3)
            adjusted_params["quality_window_size"] = max(7, self.quality_window_size - 2)
            adjusted_params["quality_threshold_fraction"] = max(0.6, self.quality_threshold_fraction - 0.1)
            ...
            adjusted_params["quality_threshold_fraction"] = max(0.65, self.quality_threshold_fraction - 0.05)
            ...
            adjusted_params["quality_window_size"] = self.quality_window_size
            adjusted_params["quality_threshold_fraction"] = self.quality_threshold_fraction
            adjusted_params["primer_similarity_threshold"] = 0.8 if not self.adna_damage_mode else 0.5
    ...
    def _create_damage_summary(self, processing_stats: Dict) -> Dict:
        ...
        return {
            "damage_progression": {
                ...
                "after_trimming_pattern": after_trimming.get("damage_pattern", "unknown"),
            ...
            "extremity_analysis": {
                ...
                "five_prime_improvement": original.get("five_prime_n_fraction", 0.0) - final.get("five_prime_n_fraction", 0.0),
                "three_prime_improvement": original.get("three_prime_n_fraction", 0.0) - final.get("three_prime_n_fraction", 0.0)
            },
            "processing_effectiveness": {
                "primer_removal_effective": after_primers.get("total_n_fraction", 1.0) < original.get("total_n_fraction", 1.0),
                "trimming_effective": after_trimming.get("total_n_fraction", 1.0) < after_primers.get("total_n_fraction", 1.0),
                "overall_improvement": original.get("total_n_fraction", 1.0) - final.get("total_n_fraction", 1.0)
            }
        }

    def process_ab1_file_enhanced(
        self, ab1_file: Path, fasta_output: Path, processed_output: Path, plot_output: Path
        ...
        if self.adaptive_quality_threshold:
            ...
            if adjusted_params.get("adjusted", False):
                current_min_quality = adjusted_params.get("min_quality", self.min_quality)
                current_window_size = adjusted_params.get("quality_window_size", self.quality_window_size)
                current_threshold_fraction = adjusted_params.get("quality_threshold_fraction", self.quality_threshold_fraction)
                logger.info(f"Adjusted parameters for {n_analysis_original['damage_pattern']}: Q{current_min_quality}, window={current_window_size}, fraction={current_threshold_fraction:.2f}")
            else:
        ...
        if hvs_region and self.enable_primer_removal:
            primer_sequence, primer_removal_info = self.remove_primers_with_orientation(sequence, hvs_region)
            processing_stats["primers_removed"] = primer_removal_info.get("primers_removed", False)
            ...
            if primer_removal_info.get("primers_removed", False):
                ...
                logger.info(f"Primers removed: F={primer_removal_info.get('forward_removed', False)}, R={primer_removal_info.get('reverse_removed', False)}")
                logger.info(f"Length reduction: {primer_removal_info.get('length_reduction', 0)} bases")
        else:
            processing_stats["primers_removed"] = False
            processing_stats["primer_removal_info"] = {"primers_removed": False, "reason": "disabled_or_no_region"}
        
        # Analyze N abundance after primer removal
        n_analysis_after_primers = self.analyze_extremity_n_abundance(sequence, "after_primers")
        ...
        filtered_seq = "".join(
            [base if qual >= current_min_quality else "N" for base, qual in zip(sequence, qualities)]
        )
        ...
        processing_stats["damage_assessment"] = self._create_damage_summary(processing_stats)

        if valid_bases < self.min_sequence_length:
            logger.warning(f"Sequence {raw_record.id} too short after processing: {valid_bases} valid bases (minimum: {self.min_sequence_length})")
        ...
        logger.info(f"Wrote processed FASTA: {processed_output} ({valid_bases} valid bases)")
    ...
    def detect_hvs_region(self, sequence: str) -> Optional[str]:
        ...
        for hvs_region, primers in self.primers.items():
            ...
            reverse_primer = primers['reverse_complement']  # Use reverse complement for matching
            
            # Score based on best primer matches
            forward_score = self._find_best_primer_match(seq_upper, forward_primer, from_start=True)
            reverse_score = self._find_best_primer_match(seq_upper, reverse_primer, from_start=False)
            
            # Combined score (average of both primers)
            combined_score = (forward_score + reverse_score) / 2
            
            if combined_score > best_score and combined_score > 0.35:  # 35% minimum similarity
                best_score = combined_score
                best_region = hvs_region
                
        return best_region

    def _find_best_primer_match(self, sequence: str, primer: str, from_start: bool = True) -> float:
    ...
    def generate_quality_plot(self, record: SeqRecord, output_file: Path, figure_size: Tuple[int, int] = (12, 4)) -> None:
        ...
        plt.axhline(self.min_quality, color="red", linestyle="--", label=f"Quality threshold ({self.min_quality})")
