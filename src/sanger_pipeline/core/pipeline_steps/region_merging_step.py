...
class RegionMergingStep:
    ...
    def execute(self, directories: Dict[str, Path]) -> Dict[str, int]:
        ...
        """
        logger.info(
            "Step 3: Merging available HVS regions into final consensus sequences"
        )
        ...
        for sample_name, hvs_files in sample_consensus.items():
            ...
            try:
                ...
                processed_samples += 1
                logger.debug(
                    f"Created merged sequence for {sample_name}: {merged_name}"
                )
    ...
    def _group_consensus_by_sample(
        self, consensus_files: list
    ...
    def _create_merged_sequence(
        self, sample_name: str, available_regions: list, sequences: Dict[str, str]
    ) -> tuple:
        """
    ...
    def _write_merged_consensus(
        self, final_dir: Path, merged_name: str, merged_sequence: str
    ) -> None:
        """
