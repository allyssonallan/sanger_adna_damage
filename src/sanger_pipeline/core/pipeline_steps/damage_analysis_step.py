...
class DamageAnalysisStep:
    ...
    def execute(self, directories: Dict[str, Path]) -> Dict[str, int]:
        ...
        ref_file = (
            directories.get("ref", directories["output"].parent / "ref") / "rCRS.fasta"
        )
        if not ref_file.exists():
            logger.warning(
                f"Reference file not found: {ref_file}. Skipping damage analysis."
            )
        ...
        for consensus_file in consensus_files:
            ...
            try:
                ...
                damage_assessment = self.damage_analyzer.assess_damage_indicators(
                    bootstrap_results
                )
                ...
                logger.error(
                    f"Failed to analyze damage patterns for {sample_name}: {e}"
                )
                continue
