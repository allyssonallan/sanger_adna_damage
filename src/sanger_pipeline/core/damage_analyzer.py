...
class DamageAnalyzer:
    ...
    def analyze_extremity_n_abundance(self, sequence: str, stage: str = "unknown") -> Dict:
        ...
        damage_pattern = self._classify_damage_pattern(
            five_prime_n_percentage, 
            three_prime_n_percentage, 
            total_n_percentage
        )
    ...
    def adjust_parameters_for_damage(self, n_analysis: Dict) -> Dict:
        ...
        elif damage_pattern in ['high_bilateral_damage', 'high_five_prime_damage', 'high_three_prime_damage']:
            adjustments.update({
    ...
    def create_damage_summary(self, processing_stats: Dict) -> Dict:
        ...
        for stage in stages:
            ...
            if stage_key in processing_stats:
                ...
                summary['damage_progression'][stage] = {
                    ...
                    'three_prime_damage': n_analysis.get('three_prime_n_percentage', 0.0),
                    'total_damage': n_analysis.get('total_n_percentage', 0.0),
                    'pattern': n_analysis.get('damage_pattern', 'unknown')
                }
        
        # Determine overall assessment
        if processing_stats.get('final_n_analysis', {}).get('damage_pattern', '') in ['extreme_damage', 'high_bilateral_damage']:
        ...
        elif processing_stats.get('final_n_analysis', {}).get('damage_pattern', '') in ['moderate_damage', 'high_five_prime_damage', 'high_three_prime_damage']:
        ...
        summary['damage_indicators'] = {
            'ancient_dna_compatible': final_analysis.get('damage_pattern', '') not in ['extreme_damage'],
            'quality_concerns': final_analysis.get('total_n_percentage', 0.0) > 20.0,
            'asymmetric_pattern': 'asymmetric' in final_analysis.get('damage_pattern', ''),
