...
class PrimerHandler:
    ...
    def __init__(self, custom_primers_forward: Optional[Dict[str, str]] = None,
        ...
        """
        self.primer_pairs = self._setup_primer_pairs(custom_primers_forward, custom_primers_reverse)
    ...
    def detect_primer_orientation(self, sequence: str, region: str) -> Dict[str, Any]:
        ...
        forward_score = 0.0
        if forward_primer:
            forward_score = self._find_best_primer_match(norm_sequence, forward_primer, from_start=True)
        
        # Check for reverse primer (and its reverse complement)
        reverse_score = 0.0
        if reverse_primer:
            reverse_score = self._find_best_primer_match(norm_sequence, reverse_primer, from_start=True)
            # Also check reverse complement of reverse primer
            reverse_rc = self._reverse_complement(reverse_primer)
            reverse_rc_score = self._find_best_primer_match(norm_sequence, reverse_rc, from_start=True)
    ...
    def remove_primers_with_orientation(self, sequence: str, region: str) -> Tuple[str, Dict]:
        ...
        if orientation == 'F' and primers.get('forward'):
            ...
            primer_pos = self._find_primer_position(cleaned_sequence, primers['forward'], from_start=True)
            if primer_pos >= 0:
            ...
            primer_pos = self._find_primer_position(cleaned_sequence, primers['reverse'], from_start=True)
            if primer_pos >= 0:
    ...
    def _normalize_sequence(self, sequence: str) -> str:
        ...
        normalized = ''.join(char.upper() for char in sequence if char.upper() in 'ATCGN')
    ...
    def _find_primer_position(self, sequence: str, primer: str, from_start: bool = True) -> int:
        ...
        if from_start:
            search_range = range(0, min(search_length, len(norm_sequence) - len(norm_primer) + 1))
        else:
    ...
    def _reverse_complement(self, sequence: str) -> str:
        ...
        complement = ''.join(complement_map.get(base.upper(), base) for base in sequence)
