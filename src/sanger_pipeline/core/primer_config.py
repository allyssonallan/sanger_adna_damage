...
class PrimerConfig:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        ...
        if config_path and config_path.exists():
            ...
            if config_path:
                logger.warning(
                    f"Primer config file not found: {config_path}. Using default primers."
                )
    ...
    def load_from_yaml(self, config_path: Path) -> None:
        ...
        try:
            ...
            for region, primer_data in config.items():
                ...
                if (
                    isinstance(primer_data, dict)
                    and "forward" in primer_data
                    and "reverse" in primer_data
                ):
                    self.primers[region] = self._validate_primer_pair(
                        region, primer_data
                    )
    ...
    def validate_primers(self) -> Tuple[bool, List[str]]:
        ...
        for region, primer_data in self.primers.items():
            ...
            # Validate sequences
            forward_issues = self._validate_sequence(
                primer_data["forward"], f"{region} forward"
            )
            reverse_issues = self._validate_sequence(
                primer_data["reverse"], f"{region} reverse"
            )
    ...
    def _validate_primer_pair(
        self, region: str, primer_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if "reverse" in validated:
            validated["reverse_complement"] = self._reverse_complement(
                validated["reverse"]
            )
    ...
    def _validate_sequence(self, sequence: str, name: str) -> List[str]:
        ...
        if self.quality_control.get("warn_ambiguous_bases", True):
            ...
            if ambiguous_bases:
                issues.append(
                    f"{name}: Contains ambiguous bases: {', '.join(ambiguous_bases)} (may affect primer specificity)"
                )
    ...
    def _reverse_complement(self, sequence: str) -> str:
        ...
        complement = "".join(
            complement_map.get(base, base) for base in sequence.upper()
        )
        return complement[::-1]
...
def parse_cli_primers(primer_string: str) -> Dict[str, str]:
    ...
    for primer_def in primer_string.split(","):
        ...
        if ":" in primer_def:
            ...
            logger.warning(
                f"Invalid primer format: {primer_def}. Expected 'region:sequence'"
            )
